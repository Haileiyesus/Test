...
<body>
  ...
  <tr >
    <td colspan="2"><input name="fname2" class="box7" type="text" value="Enter name to search room..." onClick="value=''" size="26"><input name="search" type="submit" value="Search"></td>
  ...
  <tr>
    ...
    <td align="left"><input name="preferences" type="text" id="preferences" disabled="disabled">
	</td>
  </tr>
...
mysql_select_db("ethiostar",$con);
$sql="select first_name, last_name, email, res_id from reservation where first_name = '$name2'";
$result = mysql_query($sql,$con);
If(!$result)
	die("unable to query:". mysql_error());
	echo "<img src='images/white.jpg' width='35' style='float:left'><span class='style11'></span><table border='1'><tr class='style15'><th bgcolor='#F8E6C0'></th><th bgcolor='#F8E6C0'>Reserver's Name</th></tr>";
while($row=mysql_fetch_row($result)){
$id=$row[3];
echo "<tr class='style15' ><td align='center' bgcolor='#F8E6C0'><img src='images/check3.png' width='20' height='20' onClick='abc(\"$id\")'></td><td colspan='2' bgcolor='#F0F0F0'><a href='#'>";
...
mysql_close($con);
	   ...
	   function abc(x){
	   id=x;
   	    prompt_id = prompt("Enter your Reservation ID", "");
		if (id == prompt_id) {
...
if (!$con)
  ...
  $rid=$_COOKIE['res_id'];
mysql_select_db("ethiostar",$con);
$sql="select * from reservation where first_name='$fn' && res_id=''";
$result = mysql_query($sql,$con);
If(!$result)
	die("Unable to query:");
$row=mysql_fetch_row($result);
...
var preferences='<?php echo $preferences; ?>';
		...
		else alert("Wrong Reservation ID..., Please try again!");

			}
		</script>
		
		<?php
		if(isset($_POST['cancel']) && $_POST['res_id']!="")
		{
		$con=mysql_connect("localhost","root","");
		if(!$con)
		die("unable to connect");
		mysql_select_db("ethiostar",$con);
		$sql="delete from reservation where res_id='$rid'";
		$result=mysql_query($sql);
		if(!$result)
		die(mysql_error());
		echo '<script language="javascript">
alert("Registration canceled sucessfully!");</script>';
		mysql_close($con);
		}
		else if((isset($_POST['cancel']) && $res_id==""))
		echo '<script language="javascript">
alert("Please search and choose a resevation first.");</script>';
		?>	  
				<br>
		 </div>	  
		  </div>
